...
const Login = () => {
    ...
    return (
        ...
        <div className='px-8 w-full lg:w-[70%]'>
            ...
            <p className='text-3xl font-bold pb-4 pt-4'>Login</p>
            <GoogleSignIn/>
            ...
            <div className='pt-4'>
                <form onSubmit={onSubmit}>
                    ...
                    <Link className='text-right w-full float-right underline pt-2 text-gray-500 hover:text-black'>Forgot
                        Password?</Link>
                    <button
                        className='border w-full rounded-lg h-[48px] mb-4 bg-black text-white mt-4 hover:opacity-80'>Login
                    </button>
                </form>
            </div>
            {error && <p className='text-center text-lg text-red-700'>{error}</p>}
            <Link to={"/v1/register"} className='underline text-gray-500 hover:text-black'>Don’t have an account?
                Register</Link>
        </div>
    )
