        ...
        <div className='p-8 flex'>
            <div className='w-[70%]'>
                ...
                <div className='flex gap-8 flex-col'>
                   ...
                   hover:scale-110 bg-gray-200 border-gray-500 text-gray-500'><p className='text-center'>{`${new Date().getDate() + 2} / ${new Date().toLocaleString('en-US', { month: 'short' })}`}</p></div>

                            <div
                                className='w-[80px] h-[48px] flex flex-col justify-center border text-center mb-4 rounded-lg mr-4 cursor-pointer hover:scale-110 bg-white border-gray-500 text-gray-500'>
                                <p className='text-center'>{`${new Date().getDate() + 3} / ${new Date().toLocaleString('en-US', { month: 'short' })}`}</p>
                            </div>
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    <div className='mt-4 flex flex-col gap-4'>
                        <div className='flex gap-2'>
                            <input type='radio' name='payment_mathod' value={'CARD'}
                                   onChange={()=> setPaymentMethod('CARD')}/>
                            <p> Credit/Debit Card</p>
                        </div>
                        <div className='flex gap-2'>
                            <input type='radio' name='payment_mathod' value={'COD'}
                                   onChange={()=> setPaymentMethod('COD')}/>
                            <p> Cash on delivery</p>
                        </div>
                        <div className='flex gap-2'>
                            <input type='radio' name='payment_mathod' value={'UPI'}
                                   onChange={()=> setPaymentMethod('COD')}/>
                            <p> UPI/Wallet</p>
                        </div>
            ...
            <div className='w-[30%] h-[30%] border rounded-lg border-gray-500 p-4 flex flex-col gap-4'>
                <p>Order Summary</p>
                <p>Items Count = {cartItems?.reduce((total, item) => total + item.quantity, 0)}</p>
